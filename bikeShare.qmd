...
leafmap <- leaflet::leaflet() |>
    ...
    leaflet::setView(lng = chicago$long,
                     lat = chicago$lat,
                     zoom = 10) |>
    leaflet::addMarkers(lng = df$start_lng[1:50],
                        lat = df$start_lat[1:50],
                        #color = pal,
...
mapview::mapview(spData::us_states)
...
ill <- subset(spData::us_states, NAME == "Illinois" )


illgadm <- subset(usa, usa$NAME_1 == "Illinois")
...
```         
bikeshare-research.org/api/v1/systems/chicago
```

```{r}

req <- curl::curl_fetch_memory(
    "https://bikeshare-research.org/api/v1/systems/chicago")

response_body <- rawToChar(req$content)

json_data <- jsonlite::fromJSON(response_body)

json_data
```

[Bike Share Map](https://bikesharemap.com/#/8/-87.5771/41.3747/)

[CityBikes: bike sharing networks around the world](https://citybik.es/)
...
my_series <- pd$Series(c(2,4,6,8), index = c('blue',
                                             'red',
                                             'green',
                                             'yellow'))
